...
/*--------------------------------------------------------------------------*/
{
  FILE *fpFlatFileMod, *fpFlatFile, *fpReprocessFlatFile;
  ...
  char reprocessedFlatFile[200], flatFile[200], flatFileMod[200];
...
/* CHECK INPUT ARGUMENTS */
  if ( argc != 4 ) {
    fprintf(stderr,"USAGE: %s [flatfile] [reprocessed flatfile] [output flatfile]\n", argv[0]);
    fprintf(stderr,"Relocated catalog file, e.g., emm_c2_OK_KS_201702_add_all2.csv\n");
    exit(1);
  }
  sscanf(argv[1],"%s", flatFile);
  sscanf(argv[2],"%s", reprocessedFlatFile);
...
// Open flatfiles
  ...
  if ((fpReprocessFlatFile = fopen(reprocessedFlatFile, "r")) == NULL) {
    fprintf(stderr,"Could not open flatfile, %s\n", reprocessedFlatFile);
    exit(0);
  }
  else {
...
// LOOP THROUGH ALL LINES IN REPROCESSED FLATFILE
    objMisfit=5;
// header lines
    fgets(buff2,BUFFLEN,fpReprocessFlatFile);
    fgets(buff2,BUFFLEN,fpReprocessFlatFile);
    while( fgets(buff2,BUFFLEN,fpReprocessFlatFile) ) {
//      strip(buff2);
      buff2[strcspn(buff2, "\r\n")] = 0;
      columns2 = NULL;
...
//fprintf(stderr,"%s\n",buff2);
      assign_cols_flatfile_reprocess(columns2, &stLat2, &stLon2, &evMag2, &evLon2, &evLat2, &evDep2, &evYear2, &evMon2, &evDay2, &evHour2, &evMin2, &evSec2, network2, stationNm2);
      free(columns2);
